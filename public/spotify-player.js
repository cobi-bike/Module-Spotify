class SpotifyPlayer {
  ...
  login() {
    return new Promise((resolve, reject) => {
      ...
      const url = getLoginURL(['user-read-playback-state', 'user-modify-playback-state', 'playlist-modify-private', 'playlist-modify-public', 'playlist-read-private', 'playlist-read-collaborative']);
  ...
  login2() {
    return new Promise((resolve, reject) => {
      ...
      const url = getLoginURL(['user-read-playback-state', 'user-modify-playback-state', 'playlist-read-private', 'playlist-read-collaborative']);
  ...
  fetchPlaylists(list, next) {
    ...
    if (next) url = next;
        ...
        return this.fetchGeneric(url).then(response => {
      if (response.status === 401) {
        return this.fetchToken()
          .then(tokenResponse => {
            if (tokenResponse.status === 200) {
  ...
  findOrCreateCobiPlaylist() {
    ...
    if (cobiPlaylist) {
      ...
      return this.createCobiPlaylist()
      .then(() => {
